...
![엔진 부위 설명.jpg](<attachment:엔진 부위 설명.jpg>)
...
[FD-001 문제해결 핵심 전략]    
   ...
   - RUL Clipping
      ...
      4. 실제 RUL보다 더 큰 RUL을 모델이 예측하게 되면 late prediction으로 인해 nasa score가 매우 나빠지는데 clipping으로 한계점을 만들어줄수 있습니다.
            ...
            1) 고정 클리핑: 경험에서 도출된 값인 120~130으로 RUL을 고정 클리핑 (Heimes et al. 2008 우승 논문에서 125~130 사용)     
                  장점: 단순하고 노이즈에 강하다      
                  단점: 엔진 간의 최대 RUL의 차이가 클 경우 긴 수명의 엔진이 제대로 예측되지 않아 예측의 정확도가 낮아진다.

            2) CUSUM: 초기 정상 상태의 평균값에서 벗어나는 오차를 누적하다가 특정 임계치를 넘는 순간을 감지한다.    
                  장점: 미세한 변화를 감지하는데에 탁월하다. 엔진별로 별개 RUL 클리핑 할 경우 사용가능하다.      
                  단점: 변화가 발생 이후 감지되기 때문에 반응이 느려 late prediction의 원인이 된다.

            3) CV 기반 최적화: Clipping Ppint를 하이퍼파라미터로 취급하고 특정 값을 입력해 최고 nasa score가 나오는 최적 포인트를 찾는다.     
                  장점: 수학적으로 가장 적합한 값을 찾는다.     
                  단점: 트레인 데이터에 과적합해 테스트 데이터 예측 정확도가 쩔어진다.

            4) 이외에 기하학적 특징을 사용하는 kneed point 등이 있다.
